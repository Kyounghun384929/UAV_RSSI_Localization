...
class GPSDeniedENV():
    ...
    def InitTargetCount(self, threshold_rssi=-95):
        data = {}
    ...
    def step(self, action):
        ...
        for obj in self.ground_objects:
            ...
            if len(obj) >= 8: # (기본 5개 + 위치&rssi 3개)
                ...
                x, y = multilateration(positions, self.n)
                
                data = {
                    'id': obj['id'],
                    'estimated pos': (x, y)
                }
                self.estimated_pos.append(data)
    ...
    def get_reward(self, state):
        ...
        for obj in self.ground_objects:
            if f'Aps_{self.visited}' in obj:
                ...
                error = self.initial_scan_target_count.get(state, 0)
            total_error += error

        if estimated_count > 0:  # 최소 한 개 이상의 객체 위치 추정
            avg_error = total_error / estimated_count
            return -avg_error  # 오차 최소화 (음수 보상)
        else:
            return self.initial_scan_target_count.get(state, 0)  # 초기 스캔 결과 기반 보상
...
class QAgent():
            ...
            while not stop:
                ...
                if stop:
                    if episode+1 == 1 or episode % 1000 == 0:
                        ...
                        for i, cell in enumerate(self.env.cell_points):
                            if i == 0:
                                plt.scatter(cell[0], cell[1], c='#00FF6A', marker='^', label='Cell Waypoint')
                            else:
                                plt.scatter(cell[0], cell[1], c='#00FF6A', marker='^')
    ...
    def save_q_table(self, filename="q_table_addwork.json"):
        """
